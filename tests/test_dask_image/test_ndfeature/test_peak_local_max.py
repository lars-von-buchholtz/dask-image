...
def test_peak_local_max_2d(shape, points, min_distance, footprint, threshold_abs, num_peaks):
    a = make_img(shape, points)
    chunks = [e//2 for e in shape]
    d = da.from_array(a, chunks=chunks)
    ski_r = ski_plm(a, min_distance=min_distance, threshold_abs=threshold_abs,
    ...
    da_r = da_plm(d,  min_distance=min_distance, threshold=threshold_abs,
...
@pytest.mark.parametrize("shape, points", [((50, 50, 20), np.array([[10, 10, 5, 0.7]])),
...
def test_peak_local_max_nd(shape, points, min_distance, footprint, threshold_abs, num_peaks):
    a = make_img(shape, points)
    chunks = [e//2 for e in shape]
    d = da.from_array(a, chunks=chunks)
    ski_r = ski_plm(a, min_distance=min_distance, threshold_abs=threshold_abs,
